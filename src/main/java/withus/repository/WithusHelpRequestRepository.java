...
@Repository
public interface WithusHelpRequestRepository extends JpaRepository<WithusHelpRequest, Integer> {
    /**
     * 향후 위더스 도우미 호출 내역 중 처리되지 않은 것을
     * 리스팅 할 때 쓸 수 있을 것만 같은 method
     */
    List<WithusHelpRequest> findAllByDealtWithIsFalseOrderByDateTime();
     ...
     * @param end   검색 종료 일시
     */
    List<WithusHelpRequest> findAllByDateTimeIsBetween(LocalDateTime start, LocalDateTime end);
     ...
     * @param year 검색 연도
     */
    default List<WithusHelpRequest> findAllByYear(int year) {
     ...
     */
    default List<WithusHelpRequest> findAllByMonth(int year, int month) {
     ...
     * @param day   검색 일 (1...31)
     */
    default List<WithusHelpRequest> findAllByDay(int year, int month, int day) {
